...
public:
    ...
    std::vector<std::vector<std::string>> Reversible_Transitions;
    std::vector<std::vector<longint_ARRAY_pair>> Reversible_Transitions_NodeStates; // each element of the vector is a list of value pairs: (nodeID, value)
    std::vector<std::vector<bool>> Reversible_Transitions_LimitCycle;

    Phenotypes_for_Stats_v2 (){}
...
struct Rev_Transition_stats{
    unsigned long int *N_event_to_phen, *N_event_leaving_phen, Rev_Jumps;
    doublepointer *Rev_Trans_Matrix;
    unsigned long long int T_live;
    double *av_window_in_phen,*SD_window_in_phen,*Half_life;
    unsigned int *T_in_phen;
//    unsigned int *Dist_Time_in_phen[MAX_ARREST+1];
};
