...
#define T_max_CellCycleSample   250  // max length of single thread
#define T_max_TransitionsSample    500000 // total live cell time
#define BOX_ENV    20
#define BOX_KO_OE   10
...
#include "Python_Figures_module_v2.h"
#include "Timecourses_and_stats_module_v2.h"
...
void NonSaturating_Stats_FIX_KDOE_fn1Env(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD, const char linenow[],int run_assync){
   // 2 options!!!
    ...
    load_Phenotypes_for_model_v2(PD);
    ...
    for(double p_env=0;p_env<=1.; p_env+=1./(double)BOX_ENV){
        ...
        if(cc_stats!=Phen_STATs_v2.CellCycleErrors) CC_run_stochastic_inputs_and_KO_OE_hits_STATS_SWEEP_on_attractor_list(datadir,EnvHit,l,PD, run_assync);
        if(irrev_stats!=Phen_STATs_v2.Irrev) Irrev_run_stochastic_inputs_and_KO_OE_hits_STATS_SWEEP_on_attractor_list(datadir,EnvHit,l,PD, run_assync);
        if(rev_stats!=Phen_STATs_v2.Rev) RevTrans_run_stochastic_inputs_and_KO_OE_hits_STATS_SWEEP_on_attractor_list(datadir,EnvHit,l,PD, run_assync);
    }
    
    if(((cc_stats!=Phen_STATs_v2.CellCycleErrors))||((irrev_stats!=Phen_STATs_v2.Irrev))||((rev_stats!=Phen_STATs_v2.Rev))) {
        Python_Grapsh_NonSaturating_Stats_FIX_KDOE_fn1Env(input_index,EnvHit,l,PD, run_assync,&cc_stats,&irrev_stats,&rev_stats);
...
void run_Systemic_Damage(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD, const char linenow[]){
    longint_ARRAY *l, cg;
    char fnd[600];
    l=get_requested_cell_group(PD,linenow);
    double Dp_fade=0, p_fadebump=0, Dp_rec=0, p_recbump=0;
    unsigned int T_damage=0;
    char attr_list[200][5000];

    sprintf(fnd,"mkdir %s/%s/_EXP\n",MODEL_Directory_system,PD->BRN->path);
    system(fnd);
    sprintf(fnd,"mkdir %s/%s/_EXP/Systemic_Damage",MODEL_Directory_system,PD->BRN->path);
    system(fnd);
    
    // (Dp_fade 0.01, p_fadebump 0.9, T_damage 100, Dp_rec 0.01, p_recbump 0.9)
    ...
    if (Nf == 5){
        istringstream iss(attr_list[1]);
        iss >> bla2;
        iss >> Dp_fade;
        if (iss.fail() == true){
            printf("Invalid Dp_fade in %s\n",attr_list[1]); getchar();
        }
        istringstream iss1(attr_list[2]);
        iss1 >> bla2;
        iss1 >> p_fadebump;
        if (iss1.fail() == true){
            printf("Invalid p_fadebump in %s\n",attr_list[2]); getchar();
        }
        istringstream iss2(attr_list[3]);
        iss2 >> bla2;
        iss2 >> T_damage;
        if (iss2.fail() == true){
            printf("Invalid T_damage in %s\n",attr_list[3]); getchar();
        }
        istringstream iss3(attr_list[4]);
        iss3 >> bla2;
        iss3 >> Dp_rec;
        if (iss3.fail() == true){
            printf("Invalid Dp_rec in %s\n",attr_list[4]); getchar();
        }
        istringstream iss4(attr_list[5]);
        iss4 >> bla2;
        iss4 >> p_recbump;
        if (iss4.fail() == true){
            printf("Invalid p_recbump in %s\n",attr_list[5]); getchar();
        }
    }
    else {
        printf ("Systemic_Damage line \n\t%s\nincorrect, specify as \n\t Systemic_Damage 143    (Dp_fade 0.01, p_fadebump 0.9, T_damage 100, Dp_rec 0.01, p_recbump 0.9)\nwith 5 parameters (ignoring line)\n",linenow);
        return;
    }
    ...
    if (l!=NULL){
        ...
        for(int j=1;j<=l->N;j++){
            ...
            PD->Coupled->BRN->BN->set_zero_link_weights();

            unsigned long int a_st_k = rand_int(1, PD->Coupled->Attractor_valleys[l->A[j]]->N_a);
            PD->Coupled->set_state(PD->Coupled->Attractor_valleys[l->A[j]]->Basin_Stored[PD->Coupled->Attractor_valleys[l->A[j]]->Attractor[a_st_k]]->s);
            
            long int DMAX=PD->Coupled->Attractor_valleys[l->A[j]]->N_a;
            int N_lines=PD->BRN->N+10;
            ...
            sprintf(fn,"%s/%s/_EXP/Systemic_Damage/Systemic_Damage_experiment_A%ld__Dp_fade-%.3lf__p_fadebump-%.3lf__T_damage-%u__Dp_rec-%.3lf__p_recbump-%.3lf.eps", MODEL_Directory,PD->BRN->path,l->A[j],Dp_fade, p_fadebump, T_damage , Dp_rec , p_recbump );
            printf("%s\n",fn);
            f=fopen(fn,"w");
            ...
            long int recovery_time = timetrace_to_cytokinesis_link_damage(PD,Dp_fade, p_fadebump, T_damage , Dp_rec , p_recbump ,f,YTOP,W);
            
            
            sprintf(fn,"%s/%s/_EXP/Systemic_Damage/Systemic_Damage_experiment_A%ld__Dp_fade-%.3lf__p_fadebump-%.3lf__T_damage-%u__Dp_rec-%.3lf__p_recbump-%.3lf", MODEL_Directory,PD->BRN->path,l->A[j],Dp_fade, p_fadebump, T_damage , Dp_rec , p_recbump );
            close_EPSDRAW_file(f,fn,2);
            
            
            unsigned long int *recovery_dist;
            recovery_dist = new unsigned long int[TIMESTEPS_DEFAULT+1];
            for(unsigned int s=0;s<=TIMESTEPS_DEFAULT;s++) recovery_dist[s]=0;
            ...
            for(unsigned int s=1;s<=10000;s++){
                ...
                unsigned long int a_st_k = rand_int(1, PD->Coupled->Attractor_valleys[l->A[j]]->N_a);
                PD->Coupled->set_state(PD->Coupled->Attractor_valleys[l->A[j]]->Basin_Stored[PD->Coupled->Attractor_valleys[l->A[j]]->Attractor[a_st_k]]->s);
                recovery_time = measure_time_to_cytokinesis_link_damage(PD,Dp_fade, p_fadebump, T_damage , Dp_rec , p_recbump);
                //printf("%d \t recovery = %d\n",s,recovery_time); //getchar();
                if(recovery_time>0) { recovery_dist[recovery_time]+=1;N_rec++;}
            }
            if(N_rec>0){
                sprintf(fn,"%s/%s/_EXP/Systemic_Damage/Systemic_Damage_REC_TIME_DIST_A%ld__Dp_fade-%.3lf__p_fadebump-%.3lf__T_damage-%u__Dp_rec-%.3lf__p_recbump-%.3lf.txt", MODEL_Directory,PD->BRN->path,l->A[j],Dp_fade, p_fadebump, T_damage , Dp_rec , p_recbump );
                f=fopen(fn,"w");
                fprintf(f, "#Rec_time\tN_cells\n");
                    for(unsigned int s=1;s<=TIMESTEPS_DEFAULT;s++)
                        if (recovery_dist[s]>0)    fprintf(f, "%d\t%lf\n",s,recovery_dist[s]/(double)N_rec);
                fclose(f);
            }
        }
    }
...
void run_Phenotype_Change_in_Mutants_Across_Phase_Space(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD, const char linenow[],int run_assync){
    longint_ARRAY *l;
    Environment_and_hits *EnvHit;
    l=get_requested_cell_group(PD,linenow);
    
    istringstream iss(linenow);
    string sr,inputname_1,inputname_2,inputname_3;
    iss >> sr; iss >> sr;
    ...
    iss >> inputname_3;
    unsigned long int input_id_3=PD->Coupled->BRN->MDAT->get_node_ID(inputname_3.c_str());
    if (input_id_3<1) {
        printf ("Third input node %s from virtual experiment line %s is not in Boolean model! (ignoring line)\n",inputname_3.c_str(),linenow);
        return;
    }
    
     
    unsigned long int input_index_3 =PD->Coupled->BRN->active_inputs->check_for_element(input_id_3);
    if (input_index_3<1) {
        printf ("Input node %s from virtual experiment line %s is a node, but not an INPUT of this Boolean model! (ignoring line)\n",inputname_3.c_str(),linenow);
        return;
    }
    ...
    bool cc_stats,irrev_stats,rev_stats;
    Python_Grapsh_NonSaturating_Stats_Scan_1KDOE_3Env(input_index_1,input_index_2,input_index_3,EnvHit,l,PD, run_assync,&cc_stats,&irrev_stats,&rev_stats);
