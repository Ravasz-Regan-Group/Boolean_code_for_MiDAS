...
int draw_timestep_MOD(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD,int t,
...
long int timetrace_to_cytokinesis_link_damage(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD, double Dp_fade,double p_fadebump, unsigned int T_damage , double Dp_rec ,double p_recbump, FILE *f,double y0,int W){
    int T_MAX=TIMESTEPS_DEFAULT,TW=3,NameBuff=55;
    long int tc=0;
    int Pmax=0;
    int SHOW_Init_state = 50;
    
    int id_cytokinesis=PD->BRN->MDAT->get_node_ID("Cytokinesis");
   
    for (int t=1;t<=T_MAX; t++) {
        if ((t >= SHOW_Init_state) && (t <= SHOW_Init_state+T_damage))
            PD->Coupled->BRN->BN->UP_all_link_weights(Dp_fade,p_fadebump);
        else {
            if (t > SHOW_Init_state+T_damage)
                PD->Coupled->BRN->BN->DOWN_all_link_weights(Dp_rec,p_recbump);
            }

        PD->Coupled->synchronously_update_all_Gates_dissapearing_links();
        Pmax= draw_timestep_MOD(PD,t,f,y0,W, T_MAX, TW, NameBuff);
        
        if ((t > SHOW_Init_state+T_damage) && ( PD->Coupled->s[id_cytokinesis-1]==49) && (tc==0))
            tc = t - SHOW_Init_state - T_damage;
    }
    EpsSetRgb(f,0,0,0);
    EpsDrawRectangle(f,NameBuff + SHOW_Init_state * TW, y0-0*W-W, NameBuff + (SHOW_Init_state+T_damage+1)*TW, y0-Pmax*W);
    if(tc>0) return (tc);
    else return (-1);
}

long int measure_time_to_cytokinesis_link_damage(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD, double Dp_fade,double p_fadebump, unsigned int T_damage , double Dp_rec ,double p_recbump){
