...
public:	
   ...
   // State_index_MAP_type Transition_track;
    ...
    char update_Gate_dissapearing_links(unsigned long long node_ID){
        snode::slinklist::slink *sc2;
        unsigned long long k_1;
        unsigned int k;
        char c;
        ...
        while(sc2!=NULL){
            k++;
            pok = rand_real(0, 1);
            if((s[sc2->sneighbor-1]=='1') && ((pok > sc2->link_props->lw) || (BRN->active_inputs->check_for_element(sc2->sneighbor)))) // link survives or exempt from the probabilitic disspearance of its source
                k_1+=(unsigned long long)(pow(2.,(double)BRN->BN->node[node_ID].k_out-k));
            // if(BRN->N>10) printf("%lld:%d ",sc2->sneighbor,s[sc2->sneighbor-1]);
            sc2=sc2->next(sc2);//getchar();
        }
    ...
    void Asynchronously_update_all_Gates_dissapearing_links(){
        unsigned int i,l;
        double *x;
        ...
        for(l=1;l<=BRN->N;l++){
            i=(unsigned int)(x[l]);  // order goes from 1 to N
            s[i-1]=update_Gate_dissapearing_links(i);    // index order goes from 0 to N-1 but gate index is 1 to N
        }
        s[BRN->N]=0;
        delete[] x;x=NULL;
    }
    ...
    void Asynchronously_update_all_Gates_dissapearing_links(longint_ARRAY_pair *Hits, double p_hits[]){
       unsigned int i,l;
       double *x;
       ...
       for(l=1;l<=BRN->N;l++){
           i=(unsigned int)(x[l]);  // order goes from 1 to N
           if(Hits!=NULL){
               if(skip->check_for_element(i)==0) s[i-1]=update_Gate_dissapearing_links(i);
           }
           else s[i-1]=update_Gate_dissapearing_links(i);    // index order goes from 0 to N-1 but gate index is 1 to N
       }
       s[BRN->N]=0;
       delete skip; skip = NULL;
       delete[] x;x=NULL;
    ...
    void Asynchronously_update_all_Gates_dissapearing_links(int preserve_order,longint_ARRAY_pair *Hits, double p_hits[]){
        unsigned int i,l,a;
        double *x;
    ...
    void synchronously_update_all_Gates_dissapearing_links(){
        char *s_n;
        unsigned int i;
        
        s_n=new char[BRN->N+1];
        for(i=0;i<BRN->N;i++)
            s_n[i]=update_Gate_dissapearing_links(i+1);
        for(i=0;i<BRN->N;i++) s[i]=s_n[i];
        s[BRN->N]=0;
        delete[] s_n;s_n=NULL;
    }
