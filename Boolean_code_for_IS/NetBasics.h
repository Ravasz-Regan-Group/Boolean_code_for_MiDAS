...
class sgraph {
    ...
    */
    ...
    unsigned short int add_link_with_label_append(unsigned long long id1,unsigned long long id2,double w,char la[]){
        ...
        void set_zero_link_weights(){
            snode::slinklist::slink *sc2;
            unsigned long long i;
            
            for(i=1;i<=N;i++){
                sc2=node[i].Li.first_link_pt;
                while(sc2!=NULL){
                    if(sc2->link_props==NULL) sc2->link_props=new link_attr(0);
                    else sc2->link_props->set_link_weigth(0);
                    sc2=sc2->next(sc2);
                }
            }
        }
    
        void UP_all_link_weights(double dp, double prob_bump){
            snode::slinklist::slink *sc2;
            unsigned long long i;
            double lw;
            ...
            for(i=1;i<=N;i++){
                sc2=node[i].Li.first_link_pt;
                while(sc2!=NULL){
                    if((sc2->link_props->lw<1.) &&(prob_bump > rand_real(0.,1.))) sc2->link_props->set_link_weigth(sc2->link_props->lw + dp);
                    if (sc2->link_props->lw > 1.) sc2->link_props->set_link_weigth(1.);
                    sc2=sc2->next(sc2);
                }
            }
        }
    
        void DOWN_all_link_weights(double dp, double prob_bump){
            snode::slinklist::slink *sc2;
            unsigned long long i;
            double lw;
            ...
            for(i=1;i<=N;i++){
                sc2=node[i].Li.first_link_pt;
                while(sc2!=NULL){
                    if((sc2->link_props->lw>0.) &&(prob_bump > rand_real(0.,1.))) sc2->link_props->set_link_weigth(sc2->link_props->lw - dp);
                    if (sc2->link_props->lw < 0.) sc2->link_props->set_link_weigth(0.);
                    sc2=sc2->next(sc2);
                }
            }
        }
