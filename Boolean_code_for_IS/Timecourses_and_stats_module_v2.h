...
Environment_and_hits *extract_environment_and_hits(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD, const char linenow[]){
    ...
    char bla2[1000];unsigned long int Nf=0;
    if(word.size()>0) {
...
longint_ARRAY_pair *extract_sequence_of_states(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD,string inparantheses,bool *cyc){
    ...
    *cyc =true;
    ...
    char bla2[1000]; sprintf(bla2,"%s",inparantheses.c_str());
...
void load_Phenotypes_for_model_v2(MODULAR_Boolean_Dynamics_PAIRED_TRACKER *PD){
    char fn[1000];
    FILE *f;
    longint_ARRAY_pair *a,aa;
    longint_ARRAY_pair *b,bb;
    
    if(Phen_STATs_v2.WT_cell_cycle>0) return;
    ...
    sprintf(fn,"%s/%s/VirtualExp_Phenotypes_for_STATS2.txt",MODEL_Directory, PD->Coupled->SAMPLEname);
    f=fopen(fn,"r");
    if(f==NULL) {printf("VirtualExp_Phenotypes_for_STATS2.txt File is not found in Model directory %s \n \tCurrent model folder: %s\n%s\n",PD->Coupled->SAMPLEname,MODEL_Directory_system,PD->Coupled->SAMPLEname); exit(1);}
    ...
    while(fgets(GSE_line,MAX_LINE,f)!=NULL){
        ...
        if (categoryname == CC_block) {
            ...
            else {Phen_STATs_v2.WT_cell_cycle = ccl; Phen_STATs_v2.CellCycleErrors=true;}
            for(unsigned int i=1;i<=5;i++)
                if(fgets(GSE_line,MAX_LINE,f)!=NULL){
                    ...
                    if(cc == "DNA_Replication_START"){
                        Phen_STATs_v2.DNA_Replication_START = PD->Coupled->BRN->MDAT->get_node_ID(nn.c_str());
                        if(Phen_STATs_v2.DNA_Replication_START == 0) {
                            printf("Node %s, listed as a marker of %s is not in the model!\n",nn.c_str(),cc.c_str());
                            getchar();
                        }
                    }
                    if(cc == "4N_DNA"){
                        Phen_STATs_v2.DNA_Replication_DONE = PD->Coupled->BRN->MDAT->get_node_ID(nn.c_str());
                        if(Phen_STATs_v2.DNA_Replication_DONE == 0) {
                            printf("Node %s, listed as a marker of %s is not in the model!\n",nn.c_str(),cc.c_str());
                            getchar();
                        }
                    }
                    if(cc == "Metaphase_START"){
                        Phen_STATs_v2.Metaphase_START = PD->Coupled->BRN->MDAT->get_node_ID(nn.c_str());
                        if(Phen_STATs_v2.Metaphase_START == 0) {
                            printf("Node %s, listed as a marker of %s is not in the model!\n",nn.c_str(),cc.c_str());
                            getchar();
                        }
                    }
                    if(cc == "SAC_Cleared"){
                        Phen_STATs_v2.SAC_Cleared = PD->Coupled->BRN->MDAT->get_node_ID(nn.c_str());
                        if(Phen_STATs_v2.SAC_Cleared == 0) {
                            printf("Node %s, listed as a marker of %s is not in the model!\n",nn.c_str(),cc.c_str());
                            getchar();
                        }
                    }
                    if(cc == "Cytokinesis"){
                        Phen_STATs_v2.Cytokinesis = PD->Coupled->BRN->MDAT->get_node_ID(nn.c_str());
                        if(Phen_STATs_v2.Cytokinesis == 0) {
                            printf("Node %s, listed as a marker of %s is not in the model!\n",nn.c_str(),cc.c_str());
                            getchar();
                        }
                    }
        ...
        if (categoryname == I_CC_block) {
            while ((fgets(GSE_line,MAX_LINE,f)!=NULL)&&((GSE_line[0]=='\t')||(GSE_line[0]=='-'))) {
                ...
                iss2 >> cc;
                if(cc != "--"){
                    Phen_STATs_v2.Irreversible_Do_Not_Count_CellCycle_Errors.push_back(cc); // marks each irreversible phen that should stop cell cycle counts
                    ...
                    if (a->N>0){
                         Phen_STATs_v2.Irreversible_Do_Not_Count_CellCycle_Errors_NodeStates.push_back(*a); // the whole package is added as a vector element, there is one for each phenotye
                         Phen_STATs_v2.Irreversible_Do_Not_Count_CellCycle_Errors_LimitCycle.push_back(c);
                    }
        ...
        if (categoryname == I_STAT_block) {
            while ((fgets(GSE_line,MAX_LINE,f)!=NULL)&&((GSE_line[0]=='\t')||(GSE_line[0]=='-'))) {
                ...
                iss2 >> cc;
                if(cc != "--"){
                    Phen_STATs_v2.Irreversible_STATS.push_back(cc);
                    bool c = false;
                    std::vector<std::string> word = Extract_String_Between_Delimiters_On_Same_Level(GSE_line, '(',')');
                    a=extract_sequence_of_states(PD,word[0], &c);
                    if(a->N>0){
                        Phen_STATs_v2.Irreversible_STATS_NodeStates.push_back(*a);
                        Phen_STATs_v2.Irreversible_STATS_LimitCycle.push_back(c);
                        Phen_STATs_v2.Irrev =true;
                    }
                    else {printf("Incorrectly formatted  phenotype string in the Irreversible_STATS: block %s\n\t code requires one phenotype name, then a single = character bracketed by two spaces, then a list of stable states or cycles; e.g.:\n\tEpithelial = (SNAI1=0, Twist=0, miR_34=1, Ecadherin_mRNA=1)\n\tPI3KOscillation = (p110_H=1 -> AKT_H=1 -> Nedd4L=1 -> p110_H=0 -> FoxO3=1)\t skipping line, press ENTER to continue.\n",GSE_line);
                        getchar ();
                    }
                    cc = getLastWord(GSE_line);
                    if(cc == "STOP_ALL") Phen_STATs_v2.Irreversible_STATS_GO_ON.push_back(0);
                    else {if(cc == "STOP_REVERSIBLE_COUNTS") Phen_STATs_v2.Irreversible_STATS_GO_ON.push_back(1);
                        else {if(cc == "CONTINUE_REVERSIBLE_COUNTS") Phen_STATs_v2.Irreversible_STATS_GO_ON.push_back(2);
                            else Phen_STATs_v2.Irreversible_STATS_GO_ON.push_back(0);
                        }
                    }
                  }
            }
        ...
        if (categoryname == R_STAT_block) {
            while ((fgets(GSE_line,MAX_LINE,f)!=NULL)&&((GSE_line[0]=='\t')||(GSE_line[0]=='-'))) {
                ...
                string bla;
                while ((bla.c_str()[0] != '-') && (bla != "=") && (bla != "(")){
                    iss2 >> bla; if(bla != "=") phnames.push_back(bla);
                }
                if(bla == "="){
                    std::vector<std::string> word = Extract_String_Between_Delimiters_On_Same_Level(GSE_line, '(',')');
                    if(phnames.size() == word.size()){
                        std::vector<std::string> new_module;
                        std::vector<longint_ARRAY_pair> new_module_NodeStates; // each element of the vector is a list of value pairs: (nodeID, value)
                        std::vector<bool> new_module_cycles;
                        Phen_STATs_v2.Reversible_Transitions.push_back(new_module);
                        Phen_STATs_v2.Reversible_Transitions_NodeStates.push_back(new_module_NodeStates);
                        Phen_STATs_v2.Reversible_Transitions_LimitCycle.push_back(new_module_cycles);
                        
                        unsigned long int ii=0;
                        for(ii=0;ii<phnames.size();ii++){
                            bool c1 = false;
                             a=extract_sequence_of_states(PD,word[ii], &c1);
                             if (a->N>0){
                                Phen_STATs_v2.Reversible_Transitions[Phen_STATs_v2.Reversible_Transitions.size()-1].push_back(phnames[ii]);
                                Phen_STATs_v2.Reversible_Transitions_NodeStates[Phen_STATs_v2.Reversible_Transitions.size()-1].push_back(new longint_ARRAY_pair(a));
                                Phen_STATs_v2.Reversible_Transitions_LimitCycle[Phen_STATs_v2.Reversible_Transitions.size()-1].push_back(c1);
                            }
                             else { printf("Incorrectly formatted phenotype list in %s\n\t --> length of state sequence %s smaller than 1 \n\t\t skipping line, press ENTER to continue.\n",GSE_line, word[0].c_str()); ii=phnames.size()+1;
                                 getchar ();}
                        }
                        if(ii==phnames.size()) Phen_STATs_v2.Rev = true;
                    }
                    else{
                        printf("Incorrectly formatted phenotype list in %s\n\t --> length of phenotype profile list does not match listed phenotype names: %lu names and %lu lists in () \n\t\t skipping line, press ENTER to continue.\n",GSE_line, phnames.size(), word.size()); getchar ();
                    }
                 }
                else {
                    if(bla.c_str()[0] != '-') {
                        printf("Incorrectly formatted line in the Reversible_Transitions: block: %s\n\t code requires a list of phenotype names, then a single = character bracketed by two spaces, then a matching lists of stable states or cycles; e.g.:\n\tEpithelial EM_Hybrid Mesenchymal = (miR_34=1, miR_200=1, Ecadherin_mRNA=1, ZEB1_H=0) (miR_34=0, miR_200=1, Ecadherin_mRNA=1, ZEB1_H=0, ZEB1=1, SNAI1=1) (miR_34=0, miR_200=0, Ecadherin_mRNA=0, ZEB1_H=1, N_bcatenin_H=1)\n\t\t skipping line, press ENTER to continue.\n",GSE_line);
                        getchar ();
                    }
                }
            }
    ...
    if(Phen_STATs_v2.CellCycleErrors==false) Phen_STATs_v2.WT_cell_cycle=1;
}
...
void run_stochastic_inputs_and_KO_OE_hits_experiment_on_single_attractor_MOD(
    ...
    int W=8,YTOP,XTOP;
    FILE *f;
    char fn[1000],fn1[1000],fn2[1000],fn22[1000];
...
void init_stats(Cell_cycle_Irrev_Rev_stats_in_run *Statsnow){
   // Cell_cycle_Irrev_Rev_stats_in_run Statsnow;
    ...
    for (int t=0;t<Phen_STATs_v2.Irreversible_STATS.size(); t++){
    ...
    for (int t=0;t<Phen_STATs_v2.Reversible_Transitions.size(); t++){
        Rev_Transition_stats a;
        a.T_live=0;
        a.Rev_Jumps=0;
        a.N_event_to_phen= new unsigned long int[Phen_STATs_v2.Reversible_Transitions[t].size()];
        a.N_event_leaving_phen= new unsigned long int[Phen_STATs_v2.Reversible_Transitions[t].size()];
        a.av_window_in_phen=new double[Phen_STATs_v2.Reversible_Transitions[t].size()];
        a.SD_window_in_phen=new double[Phen_STATs_v2.Reversible_Transitions[t].size()];
        a.Half_life=new double[Phen_STATs_v2.Reversible_Transitions[t].size()];
        a.T_in_phen=new unsigned int[Phen_STATs_v2.Reversible_Transitions[t].size()];
        a.Rev_Trans_Matrix=new doublepointer[Phen_STATs_v2.Reversible_Transitions[t].size()];
        for(unsigned int p=0;p<Phen_STATs_v2.Reversible_Transitions[t].size();p++){
            a.N_event_to_phen[p]= 0;
            a.N_event_leaving_phen[p]= 0;
            a.av_window_in_phen[p]=0;
            a.SD_window_in_phen[p]=0;
            a.Half_life[p]=0;
            a.T_in_phen[p]=0;
            a.Rev_Trans_Matrix[p]= new double[Phen_STATs_v2.Reversible_Transitions[t].size()];
            for(unsigned int p2=0;p2<Phen_STATs_v2.Reversible_Transitions[t].size();p2++)
                a.Rev_Trans_Matrix[p][p2]=0.;
        }
...
Cell_cycle_Irrev_Rev_stats_in_run count_for_stochastic_cell_cycles_Stochastic_HITS(Environment_and_hits *EnvHit,
    ...
    for (unsigned long long int t=1;t<=T_max_CellCycleSample; t++) { // these are the stochastic hits being scanned and/or set, collated so all are set randomly here
        ...
        for(int ii=0; ii < Phen_STATs_v2.Irreversible_Do_Not_Count_CellCycle_Errors_NodeStates.size(); ii++){ //for each phenptype that stiops the cc count
            if(check_network_state_against_stable_phenotype(PD->Coupled->s,Phen_STATs_v2.Irreversible_Do_Not_Count_CellCycle_Errors_NodeStates[ii])){
        ...
        if (r0==0){
            if(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_START-1]==49) {
                r0=1;
        ...
        if (r0==1){ // in Replication
            if(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_START-1]==49) {t_R++; }
            if(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==49) {
                r0=2;
        ...
        if(r0==2){ // 4N DNA accomplished
            if((PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==49)
                &&(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_START-1]==48)
                &&(PD->Coupled->s[Phen_STATs_v2.Metaphase_START-1]==48)
                &&(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==48))  {t_G2++; }
            
            if((t_M>0) &&(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==49)
                &&(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_START-1]==49)
                &&(PD->Coupled->s[Phen_STATs_v2.Metaphase_START-1]==48)
                &&(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==48)) {
                r0=1;
            ...
            if ((PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==49)
                &&(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_START-1]==48)
                &&((PD->Coupled->s[Phen_STATs_v2.Metaphase_START-1]==49)
                    ||(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==49))) {
                    if(t_G2>0) {
                        Statsnow.CC.Dist_G2[t_G2]++; Statsnow.CC.Time_G2+=t_G2; Statsnow.CC.N_G2++;
            ...
            if((t_G2>0)&&(PD->Coupled->s[Phen_STATs_v2.DNA_Replication_START-1]==49)) {
                r0=1;
                //  Statsnow.CC.Dist_G2[t_G2]++; Statsnow.CC.Time_G2+=t_G2; Statsnow.CC.N_G2++;
                t_G1=0;t_R=0;t_M=0;t_G2=0;t_T=0;
                t_R++;
            ...
            if ((PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==49)&&(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==49)) {r0=4;}
        }
        ...
        if(r0==4){
            if(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==48){
                if(t_M>0) {
            ...
            if((PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==49)&&(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==48)&&(PD->Coupled->s[Phen_STATs_v2.Cytokinesis-1]==49)) {
                r0=4;
                if(t_T>0) {
        ...
        if (r0==4){
            if ((PD->Coupled->s[Phen_STATs_v2.DNA_Replication_DONE-1]==48)&&(PD->Coupled->s[Phen_STATs_v2.SAC_Cleared-1]==48)&&(PD->Coupled->s[Phen_STATs_v2.Cytokinesis-1]==48)){
                r0=0;
...
void CC_run_stochastic_inputs_and_KO_OE_hits_STATS_SWEEP_on_attractor_list(
    ...
    if (l!=NULL){
        for(int j=1;j<=l->N;j++){ // for every attractor on the list of states that match the call
            ...
            //printf("%s__A%d__BOX_%d.txt",fn,(int)l->A[j],BOX_KO_OE); getchar();
            f=fopen(fn,"w");
            box=0;
            ...
            for(double p_hits=0;p_hits<=1.; p_hits+=1./(double)BOX_KO_OE){
                ...
                fprintf(f, "%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\n",
                        p_hits,
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.N_cycles/((double)Statsnow[box].CC.T_live),
                        //100*Statsnow[box].CC.N_deaths/(double)Statsnow[box].CC.T_live,
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.N_endoredupl_G2/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.N_endoredupl_Aneup/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.N_endoredupl_T/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.Time_G1/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.Time_R/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.Time_G2/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.Time_M/((double)Statsnow[box].CC.T_live),
                        Phen_STATs_v2.WT_cell_cycle * Statsnow[box].CC.Time_Telo/((double)Statsnow[box].CC.T_live));
            }
            fclose(f);
...
Cell_cycle_Irrev_Rev_stats_in_run count_irreversible_transitions_Stochastic_HITS(unsigned int vi, Environment_and_hits *EnvHit,
    ...
    for (unsigned long long int t=1;t<=T_max_CellCycleSample; t++) {
        ...
        for(int ii=0; ii < Phen_STATs_v2.Irreversible_STATS.size(); ii++){
            if((Phen_STATs_v2.Irreversible_STATS_GO_ON[ii] == 0) || ii == vi){
                if(Phen_STATs_v2.Irreversible_STATS_LimitCycle[ii]==0){
                    if(check_network_state_against_stable_phenotype(PD->Coupled->s,Phen_STATs_v2.Irreversible_STATS_NodeStates[ii])){
                        if(ii==vi){
                         ...
                         //   printf("cgecking  %s\t",Phen_STATs_v2.Irreversible_STATS[ii].c_str());
...
void Irrev_run_stochastic_inputs_and_KO_OE_hits_STATS_SWEEP_on_attractor_list(
    ...
    if (l!=NULL){
        for(int j=1;j<=l->N;j++){
            Statsnow=new Cell_cycle_Irrev_Rev_stats_in_run[BOX_KO_OE+2];
            //printf("boxp=%u\t vect = %lu\n",boxp,Phen_STATs_v2.Irreversible_STATS.size());
          //  getchar();
            for(unsigned int vi=0;vi<Phen_STATs_v2.Irreversible_STATS.size();vi++){
                ...
                sprintf(fn,"%s/Irrev_Stats_%s-",fn2,Phen_STATs_v2.Irreversible_STATS[vi].c_str());
                for(unsigned int e=1;e<=PD_A->Coupled->BRN->active_inputs->N;e++)
                    sprintf(fn,"%s_%s-%.2lf",fn,PD_A->Coupled->BRN->MDAT->Node_name[PD_A->Coupled->BRN->active_inputs->A[e]],EnvHit->p_inputs[e]);
                if(EnvHit->BKR_Hits->N>0) {
                    sprintf(fn,"%s___BKG-",fn);
                ...
                for(double p_now=0;p_now<=1.; p_now+=1./(double)BOX_KO_OE){
                    ...
                    fprintf(f, "%lg\t%lg\t%lg\t%lg\t%lg\n",
                            p_now,
                            Phen_STATs_v2.WT_cell_cycle * Statsnow[boxp].IRev[vi].N_events/((double)Statsnow[boxp].IRev[vi].T_live),
...
Cell_cycle_Irrev_Rev_stats_in_run count_Reversible_transitions_Stochastic_HITS(unsigned int vi,
    ...
    first_transition_ok=new bool [Phen_STATs_v2.Reversible_Transitions.size()];
    first_state=new unsigned int [Phen_STATs_v2.Reversible_Transitions.size()];
    previous_state=new unsigned int [Phen_STATs_v2.Reversible_Transitions.size()];
    for(int ii=0; ii < Phen_STATs_v2.Reversible_Transitions.size(); ii++) {
    ...
    for (unsigned long long int t=1;t<=T_max_CellCycleSample; t++) {
        ...
        for(int ii=0; ii < Phen_STATs_v2.Irreversible_STATS.size(); ii++){
            if(Phen_STATs_v2.Irreversible_STATS_GO_ON[ii] < 2) {
                if(Phen_STATs_v2.Irreversible_STATS_LimitCycle[ii]==0){
                    if(check_network_state_against_stable_phenotype(PD->Coupled->s,Phen_STATs_v2.Irreversible_STATS_NodeStates[ii])){
        ...
        for (unsigned int p=0; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++) {
            if(Phen_STATs_v2.Reversible_Transitions_LimitCycle[vi][p]==0){
                if(check_network_state_against_stable_phenotype(PD->Coupled->s,Phen_STATs_v2.Reversible_Transitions_NodeStates[vi][p])){
                    Statsnow.Rev[vi].T_in_phen[p]++;
                    if(first_transition_ok[vi]==true){
                            Statsnow.Rev[vi].N_event_to_phen[p]++;
                            Statsnow.Rev[vi].N_event_leaving_phen[previous_state[vi]]++;
                            Statsnow.Rev[vi].av_window_in_phen[previous_state[vi]]+=(t-t_last_transition);
                            Statsnow.Rev[vi].SD_window_in_phen[previous_state[vi]]+=(t-t_last_transition)*(t-t_last_transition);
                            Statsnow.Rev[vi].Rev_Trans_Matrix[previous_state[vi]][p]++;
                            Statsnow.Rev[vi].Rev_Jumps++;
                            previous_state[vi]=p;
                            t_last_transition = t;
                        }
                    else{
                        if(first_state[vi]==0) {first_state[vi]=p;previous_state[vi]=p;}
            ...
            for(unsigned int vi=0;vi<Phen_STATs_v2.Reversible_Transitions.size();vi++){
                ...
                for(double p_now=0;p_now<=1.; p_now+=1./(double)BOX_KO_OE){
                    ...
                    while (t_live_total<T_max_TransitionsSample) {
                        ...
                        n_runs++;
                       // printf("vi=%d\t time in 1 = %u\t  time in 2 = %u\n",vi,STnow.Rev[vi].T_in_phen[p],STnow.Rev[vi].T_in_phen[p_last]);
                       // getchar();
                        
                        Statsnow[boxp].Rev[vi].T_live+=STnow.Rev[vi].T_live;
                        Statsnow[boxp].Rev[vi].Rev_Jumps+=STnow.Rev[vi].Rev_Jumps;
                        for (unsigned int p=0; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++) {
                            Statsnow[boxp].Rev[vi].N_event_to_phen[p]+=STnow.Rev[vi].N_event_to_phen[p];
                            Statsnow[boxp].Rev[vi].N_event_leaving_phen[p]+=STnow.Rev[vi].N_event_leaving_phen[p];
                            Statsnow[boxp].Rev[vi].T_in_phen[p]+=STnow.Rev[vi].T_in_phen[p];
                            Statsnow[boxp].Rev[vi].av_window_in_phen[p]+=STnow.Rev[vi].av_window_in_phen[p];
                            Statsnow[boxp].Rev[vi].SD_window_in_phen[p]+=STnow.Rev[vi].SD_window_in_phen[p];
                            for (unsigned int p2=0; p2<Phen_STATs_v2.Reversible_Transitions[vi].size(); p2++)
                                Statsnow[boxp].Rev[vi].Rev_Trans_Matrix[p][p2]+=STnow.Rev[vi].Rev_Trans_Matrix[p][p2];
                        }
                    }
                    
                    for (unsigned int p=0; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++) {
                        if(Statsnow[boxp].Rev[vi].N_event_leaving_phen[p]>0){
                            Statsnow[boxp].Rev[vi].av_window_in_phen[p]/=(double) Statsnow[boxp].Rev[vi].N_event_leaving_phen[p];
                            Statsnow[boxp].Rev[vi].SD_window_in_phen[p]=sigma_from_average_and_sum_of_squares(
                                                   Statsnow[boxp].Rev[vi].av_window_in_phen[p],
                                                   Statsnow[boxp].Rev[vi].SD_window_in_phen[p],
                                                   Statsnow[boxp].Rev[vi].N_event_leaving_phen[p]);
                        }
                        if( Statsnow[boxp].Rev[vi].Rev_Jumps>0)
                            for (unsigned int p2=0; p2<Phen_STATs_v2.Reversible_Transitions[vi].size(); p2++)
                                Statsnow[boxp].Rev[vi].Rev_Trans_Matrix[p][p2]/=(double)Statsnow[boxp].Rev[vi].Rev_Jumps;
                    }
                }
                
                sprintf(fn,"%s/RevTransition_Stats_%s",fn2,Phen_STATs_v2.Reversible_Transitions[vi][0].c_str());
                for (unsigned int p=1; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++)
                    sprintf(fn,"%s_vs_%s",fn,Phen_STATs_v2.Reversible_Transitions[vi][p].c_str());
                ...
                sprintf(fn,"p_hit");
                for (unsigned int p=0; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++)
                    sprintf(fn,"%s\tT_in_%s\trate_to_%s_rel_CC\tAV_window_in_%s\tSD_window_in_%s\tHalfLife_%s",fn,
                        Phen_STATs_v2.Reversible_Transitions[vi][p].c_str(),
                        Phen_STATs_v2.Reversible_Transitions[vi][p].c_str(),
                        Phen_STATs_v2.Reversible_Transitions[vi][p].c_str(),
                        Phen_STATs_v2.Reversible_Transitions[vi][p].c_str(),
                        Phen_STATs_v2.Reversible_Transitions[vi][p].c_str());
                ...
                for(double p_now=0;p_now<=1.; p_now+=1./(double)BOX_KO_OE){
                   ...
                   if(Statsnow[boxp].Rev[vi].T_live==0) {
                       for (unsigned int p=0; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++) {
                           Statsnow[boxp].Rev[vi].N_event_to_phen[p]=0;
                           Statsnow[boxp].Rev[vi].N_event_leaving_phen[p]=0;
                       }
                    ...
                    fprintf(f, "%lg",p_now);
                    for (unsigned int p=0; p<Phen_STATs_v2.Reversible_Transitions[vi].size(); p++)
                        if(Statsnow[boxp].Rev[vi].T_live - Statsnow[boxp].Rev[vi].T_in_phen[p] >= 1)
                            fprintf(f, "\t%lg\t%lg\t%lg\t%lg\t%lg",
                               Statsnow[boxp].Rev[vi].T_in_phen[p]/(double)(Statsnow[boxp].Rev[vi].T_live),
                               Statsnow[boxp].Rev[vi].N_event_to_phen[p]/((double)((Statsnow[boxp].Rev[vi].T_live - Statsnow[boxp].Rev[vi].T_in_phen[p]) * Phen_STATs_v2.WT_cell_cycle)),
                                Statsnow[boxp].Rev[vi].av_window_in_phen[p],
                                Statsnow[boxp].Rev[vi].SD_window_in_phen[p],
                                Statsnow[boxp].Rev[vi].Half_life[p]);
                        else fprintf(f, "\t%lg\t%lg\t%lg\t%lg\t%lg",
                                     Statsnow[boxp].Rev[vi].T_in_phen[p]/(double)(Statsnow[boxp].Rev[vi].T_live),
                                     0.,
                                     Statsnow[boxp].Rev[vi].av_window_in_phen[p],
                                     Statsnow[boxp].Rev[vi].SD_window_in_phen[p],
